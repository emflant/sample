...
tb_time = subway1 %>% 
  filter(yeogmyeong == '여의도', gubun == '하차',
         naljja == as.Date('2022-05-01'), hoseon == 5) %>%  
...
ggplot(subway3,aes(x = times)) +
  ...
  labs(title = "geom_histogram(aes(fill = ..count..)) + facet_grid(week ~ wday)",
  ...
  theme(panel.grid.minor = element_blank(),
        ...
        plot.title = element_text(margin = margin(0.5,0,1,0, "cm"),
                                  hjust = -0.2),
...
ggsave(paste0("./test/ggplot/save_ggplot_bar_03_facet2.png"),
